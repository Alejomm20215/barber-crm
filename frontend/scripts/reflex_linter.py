...
PROBLEMATIC_PATTERNS = [
    ...
    (
        r"\b(\w+)\s*\[\s*:\s*\d+\s*\]",
        "RFX003",
        "Potential Python slicing on Reflex var. Use rx.cond or state method instead.",
    ),
    (
        r"\b(\w+)\s*\[\s*\d+\s*\](?!\s*[=,\]\)])",
        "RFX004",
        "Potential Python indexing on Reflex var. Consider using state method.",
    ),
]
...
class ReflexLinter:
    ...
    def lint_file(self, filepath: Path) -> list[LintError]:
        ...
        try:
            ...
            self.errors.append(
                LintError(file=str(filepath), line=0, column=0, code="RFX000", message=f"Could not read file: {e}")
            )
            return self.errors
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for pattern, code, message in PROBLEMATIC_PATTERNS:
                ...
                if match:
                    ...
                    col = match.start()
                    self.errors.append(
                        LintError(file=str(filepath), line=line_num, column=col + 1, code=code, message=message)
                    )
    ...
    def _check_fstrings_with_state(self, filepath: Path, content: str, lines: list[str]):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            match = re.search(pattern, line)
            if match and ("rx.text(" in line or "rx.heading(" in line or "rx.badge(" in line):
                col = match.start()
                self.errors.append(
                    LintError(
                        file=str(filepath),
                        ...
                        column=col + 1,
                        code="RFX007",
                        message="f-string with state variable in rx component. "
                        "Pass state var directly to rx.text() or use rx.text(state.var) instead.",
                    )
                )
